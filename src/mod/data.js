...
/**
 * Find or create the last patient's visit.
 */
exports.getLastVisit = function( patient ) {
    if (!Array.isArray(patient.$admissions)) {
        patient.$admissions = [];
    }
    ...
    if (!Array.isArray(admission.visits)) admission.visits = [];
    if (admission.visits.length == 0) {
        admission.visits.push({ date: Date.now(), value: {} });
    }
